...
| 项目 | 说明 |
|---|---|
| 输入形式 | 仅限链接：博主主页链接（`douyin.com/user/<sec_uid>`）或单条视频分享链接（`v.douyin.com/xxx` / `douyin.com/video/<id>`）；不支持按名字搜索，也不支持图文作品链接 |
| 登录要求 | 单作品直传免登录；批量下载或批量上传分析需先在你的 Chrome 浏览器登录抖音，并授权脚本从浏览器采集必要的会话信息 |
| 批量规模 | 建议 Top 3~5（每条约 4 分钟提取耗时），可在运行前按需调整 |
