...
description: "蒸馏抖音博主：批量下载其爆款视频并提取信息档案，交叉蒸馏出商业思维、视频思路与可直接照做的拍片行动包。当用户说'蒸馏/学习/对标某博主'、'我想做他那样的视频但不知道怎么做'，或提供博主名/主页链接要求分析打法时调用。"
---

# 博主蒸馏器（blogger-distiller）

把"看别人视频学东西"变成一条可重复的流水线：**收片 → 拆片 → 蒸馏 → 行动包**。输入一个博主（名字/主页链接），最终产出一份《蒸馏报告》——不是分析文档堆砌，而是用户能直接照着拍自己视频的行动指导。

本技能**完全自包含**：下载与提取的脚本和操作手册已内化在技能目录内，不依赖任何其他 skill。
...
1. 解析博主身份：
   - 用户给链接 → 从 URL 提取 sec_uid
   - 用户只给名字 → 按收片手册的方式导航搜索页解析（核对粉丝数/简介避免重名）
   - 用户只给本地视频目录 → 跳过收片层，直接从第 2 步开始
...
它默认把抖音签名直链直接交给方舟拉流（≤50MB 且匿名可访问时成功），失败自动回退为「下载后上传」。此脚本依赖同技能 `scripts/download/dy_dl.py`，已内置并自动定位。
...
每次运行在一个独立目录内完成（`<工作区>/distilled/<博主名>_<YYYYMMDD>/`）：
...
| 博主名重名歧义 | 列出候选账号（粉丝数/简介）让用户选择，禁止自动猜测 |
