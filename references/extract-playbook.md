...
> 内化自 ark-video-extractor 的全部操作知识。执行拆片阶段前必读。
...
默认将抖音签名直链（多候选探针择优）直接交给方舟拉流提取，不在本地落盘视频；直链不可用或超 50MB 时自动回退「CDP 下载完整 mp4 → Files API 上传」（回退临时视频提取成功后自动清理）。脚本自动在本技能 `scripts/download/` 下定位内置的 `dy_dl.py`。适用：单条视频快速拆解 / 批量流程中的单条补漏。
